...
par(mfrow = c(1,1))
plot(avg_arsenic_window, log(avg_switch_window/(1-avg_switch_window)), col="red", type="l", ylab="logit(p)")
#plot(avg_arsenic_window, avg_switch_window, col="red", type="l", ylab="p")
plot(log(avg_arsenic_window), log(avg_switch_window/(1-avg_switch_window)), col="blue", type="l", ylab="logit(p)")
...
window_size = 400
avg_switch_window <- rep(0,n-window_size+1)
avg_distance_window <- rep(0,n-window_size+1)
...
par(mfrow = c(1,1))
plot(avg_distance_window, log(avg_switch_window/(1-avg_switch_window)), col="red", type="l")
# visualize switch(log(arsenic))
plot(log(avg_distance_window), log(avg_switch_window/(1-avg_switch_window)), col="blue", type="l")
...
plot(full.log.roc, col="red", xlim = c(1,0))
...
interactions1.log <- glm(switch~.+educ*arsenic, data = ars.data, family = binomial())
...
# library(aod)
# wald.test(b = coef(interactions.log), Sigma = vcov(interactions.log), Terms = 4:6)
...
for (d in dseq) {
  ...
  if (d==dseq[1]) { plot(arsenic_range, fitted_switch, type="l", 
                         ylim=c(0,1), col=cols[which(dseq==d)], ylab="p", xlab="log(arsenic)") }
...
for (a in aseq) {
  ...
  if (a==aseq[1]) { plot(dist_range, fitted_switch, type="l", 
                         ylim=c(0,1), col=cols[which(aseq==a)], ylab="p", xlab="distance") }
...
for (e in eseq) {
  ...
  if (e==eseq[1]) { plot(arsenic_range, fitted_switch, type="l", 
                         ylim=c(0.2,1), col=cols[which(eseq==e)], ylab="p", xlab="log(arsenic)") }
...
for(i in 1:n.iter){
  ...
  full.fitted = predict(full.train.lm, newdata = test.data[,c(1,2,3,4,5)], type="response")
  ...
  almost.fitted = predict(almost.train.lm, newdata = test.data[,c(1,2,3,5)], type="response")
  ...
  interaction.fitted = predict(interaction.lm, newdata = test.data[,c(1,2,3,5,6)], type="response")
  ...
  interaction1.fitted = predict(interaction1.lm, newdata = test.data[,c(1,2,3,4,5,6)], type="response")
